...
module.exports = {
  ...
  cooldown: 3,
  aliases: ["pl"],
  description: "Odtwórz playlistę z YouTube",
  async execute(message, args) {
    ...
    if (urlValid) {
      try {
        ...
        console.error(error);
        return message.reply("Playlista nie odnaleziona :(").catch(console.error);
      }
    } else if (scdl.isValidUrl(args[0])) {
      if (args[0].includes("/sets/")) {
        message.channel.send("⌛ ładowanie playlisty...");
    ...
    message.channel.send(`${message.author} Rozpoczął/ęła odtwarzanie playlisty`, playlistEmbed);
    ...
    if (!serverQueue) {
      try {
        ...
        await channel.leave();
        return message.channel.send(`Nie mogę dołączyć na kanał: ${error}`).catch(console.error);
      }
    }
  }
};
